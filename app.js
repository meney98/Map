...
function groupHouseNames(items){
  ...
  const rows=[];
  for(const h of clean){let row=rows.find(r=>Math.abs(r.y-h.y)<=4);if(!row){row={y:h.y,items:[]};rows.push(row)}row.items.push(h)}
  const out=[];
  for(const row of rows){
    ...
    for(const h of row.items){if(g.length&&h.x-g[g.length-1].x>75)flush();g.push(h)}flush();
  }
  return out;
}

// Smaller text for longer household names so labels stay inside compact blocks.
function fontFor(name){const n=name.length;if(n>26)return 3.2;if(n>20)return 3.6;if(n>15)return 4.0;if(n>10)return 4.5;return 5.0}
