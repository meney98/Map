...
function zoom(f,cx=vp.clientWidth/2,cy=vp.clientHeight/2){const ns=Math.max(.15,Math.min(5,s*f));x=cx-(cx-x)*ns/s;y=cy-(cy-y)*ns/s;s=ns;apply()}
...
// Remove block/reference codes from the searchable/visible labels.
const isBNumber=n=>/^\(?\s*B\s*-?\s*\d+\s*\)?$/i.test(n.trim());

// The source label list contains some multi-word house names as separate words.
// Merge words that sit on the same text line and close together, so names such as
// "Blue Bell" and "Morning House" appear as one household label.
function groupHouseNames(items){
  const clean=items.filter(h=>h.name && !isBNumber(h.name));
  clean.sort((a,b)=>a.y-b.y||a.x-b.x);
  const rows=[];
  for(const h of clean){
    let row=rows.find(r=>Math.abs(r.y-h.y)<=7);
    if(!row){row={y:h.y,items:[]};rows.push(row)}
    row.items.push(h);
  }
  const out=[];
  for(const row of rows){
    ...
    for(const h of row.items){
      if(g.length && h.x-g[g.length-1].x>430) flush();
      g.push(h);
    }
    flush();
  }
...
fetch('houses.json').then(r=>r.json()).then(raw=>{
  ...
  for(const h of data){
    const a=document.createElement('span');a.className='label';a.textContent=h.name;a.dataset.name=h.name.toLowerCase();a.style.left=(h.x/10)+'px';a.style.top=(h.y/10)+'px';
    a.onclick=e=>{e.stopPropagation();q.value=h.name;search(h.name)};labels.appendChild(a)
  }
...
function search(v){
  ...
  for(const a of labels.children){
    const hit=!!v && a.dataset.name.includes(v);a.classList.toggle('hit',hit);
    // Keep the full black-and-white map visible. Search never fills a house red.
    a.style.display='block';
    if(hit&&!first)first=a;if(hit)n++;
  }
  status.textContent=v?`${n} match${n===1?'':'es'}`:`${data.length} household/place names`;
  if(first){const px=parseFloat(first.style.left),py=parseFloat(first.style.top);s=Math.max(s,2.2);x=vp.clientWidth/2-px*s;y=vp.clientHeight/2-py*s;apply()}
}
